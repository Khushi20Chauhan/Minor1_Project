...
public class Main {
    ...
    public static void cluster() {
        KMediode obj = new KMediode("C:\\Users\\NIRTO V\\OneDrive\\Desktop\\Sem-5\\Project\\Modified_data1.csv");
        Scanner sc = new Scanner(System.in);
        System.out.println("\n=============================================================================================================================================================================================================================");
        System.out.println("** Welcome to Academic Monitoring **");
        System.out.println("=============================================================================================================================================================================================================================\n");

        int k = 0;
        while (true) {
            try {
                System.out.print("Enter the number of clusters (must be between 1 and 10): ");
                k = sc.nextInt();
                if (k > 0 && k <= 10) {
                    break; // Exit loop if a valid number is entered
                } else {
                    System.out.println("Invalid input. The number of clusters must be a positive integer less than 11.");
                }
            } catch (InputMismatchException e) {
    ...
    public static void classify(){
        ...
        while (true) {
            try {
                ...
                System.out.println(space + "Error: " + e.getMessage() + " Please enter a valid name.");
            } catch (Exception e) {
                System.out.println(space + "Unexpected error. Please try again.");
            }
        }
        ...
        while (true) {
            try {
                ...
                System.out.println(space + "Error: " + e.getMessage() + " Please enter a valid SAP ID.");
            } catch (Exception e) {
                System.out.println(space + "Unexpected error. Please try again.");
            }
        }
        ...
        while (true) {
            try {
                System.out.print("\n" + space + "Enter your overall 1st year CGPA: ");
                ...
                System.out.println(space + "Error: Invalid CGPA input. Please enter a numeric value.");
            } catch (IllegalArgumentException e) {
        ...
        String[] gradePrompts = {
                ...
                "Automata and Engineering Mathematics (AEM)",
                "Physics"
        };
        ...
        for (int i = 0; i < gradeCodes.length; i++) {
            while (true) {
                try {
                    System.out.print("\n" + space + "Enter Grade for " + gradePrompts[i] + " (O, A+, A, B+, B, C+, C, D, F): ");
                    ...
                    System.out.println(space + "Error: " + e.getMessage() + " Please enter a valid grade.");
                } catch (Exception e) {
                    System.out.println(space + "Unexpected error. Please try again.");
                }
            }
        ...
        for (int i = 0; i < interestCodes.length; i++) {
            while (true) {
                try {
                    ...
                    System.out.println(space + "Error: Invalid number input. Please enter a number between 1 and 5.");
                } catch (IllegalArgumentException e) {
                    System.out.println(space + "Error: " + e.getMessage());
                } catch (Exception e) {
                    System.out.println(space + "Unexpected error. Please try again.");
                ...
                if (user != null) {
                    ...
                    if (storedPassword.equals(password)) {
                        ...
                        if (role.equals("student")) {
                            System.out.println("Login successful for student: " + userId);
                            us = userId;
                            ...
                            if (previousSpecialization != null && !previousSpecialization.isEmpty()) {
                                System.out.println("Your previous specialization suggestion was: " + previousSpecialization);
                            } else {
                                System.out.println("No previous specialization suggestion found.");
                            }
                            studentMenu();
                        } else if (role.equals("educator")) {
                            System.out.println("Login successful for educator: " + userId);
                            educatorMenu();
                        } else {
    ...
    public static void adminMenu() {
        while (true) {
            try {
                System.out.println("\nAdmin Menu:");
                System.out.println("1. Create educator account");
                System.out.println("2. See/Edit student details");
                System.out.println("3. See/Edit educator details");
                System.out.println("4. Delete educator or student");
                System.out.println("5. Classify (Student specialization suggestion)");
                System.out.println("6. Cluster (Group students based on performance)");
    ...
    public static void createEducatorAccount() {
        try {
            ...
            System.out.println("Educator account created successfully for user: " + newUserId);

        } catch (MongoException e) {
            System.out.println("Error occurred while interacting with the database: " + e.getMessage());
            e.printStackTrace();  // Print stack trace for debugging
        } catch (Exception e) {
            System.out.println("An unexpected error occurred: " + e.getMessage());
            e.printStackTrace();  // Print stack trace for debugging
    ...
    public static void manageUserDetails(String role) {
        while (true) {
            try {
                System.out.println("Search for " + role + " by:");
                ...
                if (choice == 1) {
                    // Search by User ID
                    System.out.print("Enter " + role + " User ID: ");
                    String userId = scanner.nextLine();
                ...
                if (user != null) {
                    System.out.println(role.substring(0, 1).toUpperCase() + role.substring(1) + " details: " + user.toJson());
                    // Further editing functionality can be added here, such as updating or deleting details
                } else {
                    System.out.println(role.substring(0, 1).toUpperCase() + role.substring(1) + " not found.");
                }
        ...
        while (true) {
            try {
                ...
                if (user != null) {
                    ...
                    System.out.println("User with ID " + user.getString("userId") + " deleted successfully.");
                } else {
    ...
    public static void studentMenu() {
        while (true) {
            try {
                System.out.println("\nStudent Menu:");
                System.out.println("1. Classify (Get specialization suggestion)");
    ...
    public static void educatorMenu() {
        while (true) {
            try {
                System.out.println("\nEducator Menu:");
                System.out.println("1. Cluster (Group students based on performance)");
                System.out.println("2. Exit");
                ...
                switch (choice) {
                    case 1:
                        ...
                        break;
                    case 2:
                        System.out.println("Exiting educator menu.");
                        return;
                    default:
                        System.out.println("Invalid choice. Please enter a number from the menu.");
                }
    ...
    public static void createAccount() {
        try {
            ...
            if (password.equals(confirmPassword)) {
                ...
                System.out.println(role.substring(0, 1).toUpperCase() + role.substring(1) + " account created successfully for user: " + userId);
            } else {
                System.out.println("Passwords do not match. Please try again.");
            }
        } catch (Exception e) {
